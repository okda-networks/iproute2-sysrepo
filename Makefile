...
CC        ?= gcc
LDFLAGS   = -lsysrepo -lbpf -lelf -lmnl -lbsd -lcap -lselinux -lm -g --vtv-debug
SUBDIRS   = iproute2
...
all: $(IPR2_SR_OBJ) $(IPR2_SR_LIB_OBJ) iproute2/config.mk
	...
	rm iproute2/ip/ip.o
	$(CC) -o $(BIN)/$(EXEC) -L/usr/local/lib $(IPR2_SR_OBJ) $(IPR2_SR_LIB_OBJ) `find iproute2/ip -name '*.[o]'` `find iproute2/lib -name '*.[o]'` $(LDFLAGS)
	@echo ""
	@echo "Make complete"
