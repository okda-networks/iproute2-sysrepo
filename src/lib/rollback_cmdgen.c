...
{
	if (get_extension(CMD_ADD_EXT, root_node, &oper2cmd_prefix[ADD_OPR]) !=
	    EXIT_SUCCESS) {
		fprintf(stderr,
	...
	if (get_extension(CMD_DELETE_EXT, root_node,
	...
	if (get_extension(CMD_UPDATE_EXT, root_node,
...
int lyd_leaf2arg(struct lyd_node *element, bool only_lyd_list_keys,
		 char **arg_name, char **arg_value)
{
	...
	if (element->schema->nodetype == LYS_LEAF) {
		LY_DATA_TYPE type = ((struct lysc_node_leaf *)element->schema)
					    ->type->basetype;
		if (type == LY_TYPE_IDENT) {
			*arg_value = strdup(
				get_identityref_value(lyd_get_value(element)));
		} else {
...
int lyd_leaf2rollback_arg(struct lyd_node *element, bool only_lyd_list_keys,
			  char **arg_name, char **arg_value)
{
	if (only_lyd_list_keys && !lysc_is_key(element->schema)) {
		*arg_name = NULL;
	...
	if (element->schema->nodetype == LYS_LEAF ||
		...
		if (type == LY_TYPE_IDENT) {
			*arg_value =
				strdup(get_identityref_value(rollback_value));
		} else {
...
struct lyd_node *sr_get_lyd_node_by_name(char *node_name,
					 struct lyd_node *parent_node)
{
	...
	if (sr_get_node(sr_session, xpath, 0, &node_data) != SR_ERR_OK) {
		fprintf(stderr,
			"%s: failed to get include node data from sysrepo. Node name = \"%s\"\n",
			__func__, node_name);
		return NULL;
	}
...
struct cmd_info *lyd2rollback_cmd(const struct lyd_node *startcmd_node)
{
	...
	LYD_TREE_DFS_BEGIN(startcmd_node, element)
	{
		...
		case UPDATE_OPR:
			...
			if (element->schema->nodetype == LYS_CONTAINER ||
			    element->schema->nodetype == LYS_LIST) {
				printf("node name %s\n", element->schema->name);
				if (get_extension(ON_UPDATE_INCLUDE, element,
						  &on_update_include) ==
				    EXIT_SUCCESS) {
			...
			if (element->schema->nodetype == LYS_LEAF ||
			    element->schema->nodetype == LYS_LEAFLIST) {
				lyd_leaf2rollback_arg(element, false, &arg_name,
						      &arg_value);
			}
			if (arg_name != NULL) {
